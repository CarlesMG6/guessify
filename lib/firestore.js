...
const calculateStreak = async (roomId, voterUserId, currentRoundNumber) => {
  try {
    ...
    const db = await getDb();
    const { collection, getDocs, where, query, orderBy } = await import('firebase/firestore');
    const votesRef = collection(db, 'rooms', roomId, 'votes');
    
    // Get all votes from this user in previous rounds
    const q = query(
      votesRef,
    ...
    const previousVotes = votesSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    })).sort((a, b) => b.roundNumber - a.roundNumber); // Sort descending by round
    
    // Count consecutive correct answers from most recent rounds
    let streak = 0;
