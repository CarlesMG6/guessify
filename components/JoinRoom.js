...
import { getRoom, addPlayerToRoom } from '../lib/firestore';
...
export default function JoinRoom() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!user || !spotifyUser) {
      setError('Debes estar conectado con Spotify para unirte a una room');
      return;
    }

    if (!roomCode.trim()) {
      setError('Ingresa un código de room');
      return;
    }
    ...
    try {
      ...
      if (!room) {
        setError('La room no existe. Verifica el código.');
        return;
      }
      ...
      // Add player to room
      await addPlayerToRoom(roomCode.toUpperCase(), user.uid, playerName.trim());
      ...
      setError('Error al unirse a la room. Inténtalo de nuevo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="bg-spotify-gray rounded-lg p-8">
        <h2 className="text-2xl font-bold text-white mb-6">Unirse a Room</h2>
        ...
        <div className="mt-6 pt-6 border-t border-gray-600">
          <p className="text-gray-400 text-sm text-center">
            ¿No tienes un código? Pídele al anfitrión que te comparta el código de la room.
          </p>
        </div>
      </div>
    </div>
